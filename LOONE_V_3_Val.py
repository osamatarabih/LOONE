...
def LOONE_HydNut():
    ...
    for i in range(len(Targ_Stg_df)):
        V10per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            10,
            Targ_Stg,
        )
        V20per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            20,
            Targ_Stg,
        )
        V25per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            25,
            Targ_Stg,
        )
        V30per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            30,
            Targ_Stg,
        )
        V40per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            40,
            Targ_Stg,
        )
        V45per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            45,
            Targ_Stg,
        )
        V50per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            50,
            Targ_Stg,
        )
        V60per[i] = Add_Fn.Replicate(
            Targ_Stg_df["dates"].iloc[i].year,
            Targ_Stg_df["dates"].iloc[i].timetuple().tm_yday,
            60,
            Targ_Stg,
    ...
    for i in range(n_rows - 2):
        ...
        #
        Zone_D_Rel_Code[i] = Dec_Tree_FNs.Zone_D_Rel_Code(
            Zone_D_Branch_Code[i], Pre_defined_Variables.Opt_DecTree
        )
        #
        Zone_C_Trib[i] = Dec_Tree_FNs.Zone_C_Trib(
            TC_LONINO_df.at[i, "Tributary_Condition"], Pre_defined_Variables.Opt_NewTree
        )
        #
        Zone_C_Seas[i] = Dec_Tree_FNs.Zone_C_Seas(
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"],
            Pre_defined_Variables.Opt_NewTree,
        )
        #
        Zone_C_MSeas[i] = Dec_Tree_FNs.Zone_C_MSeas(
            TC_LONINO_df.at[i, "LONINO_MultiSeasonal_Classes"]
        )
        #
        Zone_C_MetFcast[i] = Dec_Tree_FNs.Zone_C_MetFcast(
            Zone_C_Seas[i],
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"],
        ...
        #
        Zone_C_Rel_Code[i] = Dec_Tree_FNs.Zone_C_Rel_Code(
            Zone_C_Branch_Code[i], Pre_defined_Variables.Opt_DecTree
        )
        #
        Zone_B_Trib[i] = Dec_Tree_FNs.Zone_B_Trib(
            TC_LONINO_df.at[i, "Tributary_Condition"], Pre_defined_Variables.Opt_NewTree
        )
        #
        Zone_B_Stage[i] = Dec_Tree_FNs.Zone_B_Stage(
            Stage_LO[i + 1], Seasons.at[i, "Season"]
        )
        #
        Zone_B_Seas[i] = Dec_Tree_FNs.Zone_B_Seas(
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"]
        )
        #
        ZBBC = (
        ...
        #
        Zone_B_Rel_Code[i] = Dec_Tree_FNs.Zone_B_Rel_Code(
        ...
        Stage_LO[i + 2] = LO_FNs.Stage_LO(
            Stg_Sto_Ar.stg2sto(Storage[i + 2], 1),
