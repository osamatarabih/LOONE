...
def LOONE_Q(P_1,P_2,NO_1,NO_2,Chla_1,Chla_2,S77_DV,S308_DV,RegSouth_DV,P_S,NO_S,Chla_S): 
# def LOONE_Q(P_1,P_2,NO_1,NO_2,Chla_1,Chla_2,S77_DV,S308_DV,P_S,NO_S,Chla_S): 
...
##############################################################################################################    
    ...
    #Generate the Zone Column based on the corresponding Zone Code.
        ...
        if Model_Config.Sim_type == 0:
            ...
            elif (Chla_S[i] <= Chla_1) and (NO_S[i] <= NO_1) and (P_S[i] <= P_1) and (date_rng_6[i+2].month in [1,2,3,4,11,12]) and (M_var.LO_Zone[i+1] == 'B' or M_var.LO_Zone[i+1] == 'C'):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
            elif (Chla_S[i] <= Chla_2) and (NO_S[i] <= NO_2) and (P_S[i] <= P_2) and (date_rng_6[i+2].month in [5,6,7,8,9,10]) and (M_var.LO_Zone[i+1] == 'B' or M_var.LO_Zone[i+1] == 'C'):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
        ...
        if Model_Config.Sim_type == 0:
            ...
            elif (Chla_S[i] <= Chla_1) and (NO_S[i] <= NO_1) and (P_S[i] <= P_1) and (date_rng_6[i+2].month in [1,2,3,4,11,12]):
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]
            elif (Chla_S[i] <= Chla_2) and (NO_S[i] <= NO_2) and (P_S[i] <= P_2) and (date_rng_6[i+2].month in [5,6,7,8,9,10]):
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]
            elif M_var.Lake_Stage[i+1] > LO_EcoRecEnv['Upper bound'].iloc[i+1] :  # Soft constraint
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]  
            else:
