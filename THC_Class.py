...
def THC_Class(i,THC_Class_normal_or_above,Lake_O_Stage_AP,Lake_O_Schedule_Zone,LStgCorres,LowChance_Check,S77RS_AP,S77BS_AP,
              ...
              Post_AP_Baseflow_EWS_cfs,AdapProt_df,Stage_LO,Zone_Code,df_WSMs,Targ_Stg_df,S77RS,S77BS,Estuary_needs_water,
              Choose_1,WSM_Zone):
    

    THC_Class_normal_or_above[i] = AP_FNs.THC_Class_normal_or_above(AdapProt_df['Tributary Hydrologic Condition'].iloc[i],Pre_defined_Variables.THC_threshold)    
    #Lake Okeechobee Stage 
    Lake_O_Stage_AP[i] = Stage_LO[i+1]
    #Lake Okeechobee Zone Code
    Lake_O_Schedule_Zone[i] = Zone_Code[i+1]
    #
    LStgCorres[i] = AP_FNs.LStgCorres(AdapProt_df['date'].iloc[i].month,df_WSMs['WSM1'].iloc[i+1],Targ_Stg_df['%s%%' %(Pre_defined_Variables.LowChance)].iloc[i],Pre_defined_Variables.Opt_LChance_line,Pre_defined_Variables.LowChance)
    ...
    n30davgForecast[i] = AP_FNs.n30davgForecast(Estuary_needs_water['Estuary Needs Water?'].iloc[i],n30d_mavg[i:i+13],Pre_defined_Variables.OptSalFcast,Pre_defined_Variables.CE_SalThreshold)
    ...
    #
    LDS_LC6_1[i] = AP_FNs.LDS_LC6_1(AdapProt_df['Late_Dry_Season'].iloc[i],LowChance_Check[i],Pre_defined_Variables.Late_Dry_Season_Option)
    ...
    Logic_test_1[i] = AP_FNs.Logic_test_1(All_4andStage[i],P_AP_BF_Stg[i],Pre_defined_Variables.Opt_NoAP_above_BF_SB)  
    #Main Function
    Post_Ap_Baseflow[i] = AP_FNs.Post_Ap_Baseflow(Logic_test_1[i],S77BS_AP[i],All_4andStagein[i],Choose_1)
    ...
    S77RSplusPreAPS77bsf[i] = AP_FNs.S77RSplusPreAPS77bsf(S77RS_Pre_AP_S77_Baseflow[i],Lake_O_Schedule_Zone[i],Pre_defined_Variables.Opt_CEews_LOWSM)
    #AND Est Needs Lake Water?
    AndEstNeedsLakeWater[i] = AP_FNs.AndEstNeedsLakeWater(n30davgForecast[i],S77RSplusPreAPS77bsf[i])
    #AND Low chance 6/1 stage <11'?
    AndLowChance61stagelessth11[i] = AP_FNs.AndLowChance61stagelessth11(LDS_LC6_1[i],AndEstNeedsLakeWater[i])
    #AND THC normal or above?    
    ATHCnora[i] = AP_FNs.ATHCnora(AndLowChance61stagelessth11[i],THC_Class_normal_or_above[i],AdapProt_df['Late_Dry_Season'].iloc[i],Pre_defined_Variables.Opt_THCbypLateDS)
    #Post-AP EWS (cfs)
    #Define Choose_PAPEWS_1 and 2
    Choose_PAPEWS_1[i] = AP_FNs.Choose_PAPEWS_1(WSM_Zone[i+2],Pre_defined_Variables.APCB1,Pre_defined_Variables.APCB2,Pre_defined_Variables.APCB3,Pre_defined_Variables.APCB4)
    #
    Choose_PAPEWS_2[i] = AP_FNs.Choose_PAPEWS_2(Pre_defined_Variables.Opt_CEews_LOWSM,Pre_defined_Variables.CalEst_ews)
    #
    Post_AP_EWS[i] = AP_FNs.Post_AP_EWS(ATHCnora[i],WSM_Zone[i+2],Choose_PAPEWS_1[i],Choose_PAPEWS_2[i],Pre_defined_Variables.CalEst_ews)
