...
def LOONE_Q(P_1,P_2,NO_1,MO_2,Chla_1,Chla_2,S77_DV,S308_DV,RegSouth_DV,P_S,NO_S,Chla_S): 
...
##############################################################################################################    
    ...
    #Generate the Zone Column based on the corresponding Zone Code.
        ...
        if Model_Config.Sim_type == 0:
            ...
            elif (Chla_S[i] <= Chla_1) and (date_rng_6[i+2].month in [1,2,3,4,11,12]):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
            elif (Chla_S[i] <= Chla_2) and (date_rng_6[i+2].month in [5,6,7,8,9,10]):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
