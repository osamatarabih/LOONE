...
def LOONE_Constituent_SimQ(LOONE_Q_Outputs):
    #Read Required Data
    ...
    Stage = LOONE_Q_Outputs['Stage'].astype(float) * 0.3048 #m
    
    volume = LOONE_Q_Outputs['Storage'].astype(float) * 1233.48 #acft to m3
    ...
    # # Observed Stage (ft) and Storage (m3) 
    ...
    Constit_Loads_M = Constit_Loads_df.resample('M').sum()
