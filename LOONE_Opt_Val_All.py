...
#############################################################################
    Results_data = pd.read_csv('./Outputs/Dec_Var_Scen4_StLMinQ.csv')
...
##############################################################################################################    
    ...
    External_Chla_M = np.zeros(n_rows,dtype = object)

    Nit_R_N_NO = np.zeros(n_rows,dtype = object)
    Nit_R_N_T_NO = np.zeros(n_rows,dtype = object)
    Nit_R_S_NO = np.zeros(n_rows,dtype = object)
    Nit_R_S_T_NO = np.zeros(n_rows,dtype = object)
    Denit_R_N_NO = np.zeros(n_rows,dtype = object)
    Denit_R_N_T_NO = np.zeros(n_rows,dtype = object)
    Denit_R_S_NO = np.zeros(n_rows,dtype = object)
    Denit_R_S_T_NO = np.zeros(n_rows,dtype = object)
    

    fT_NO = np.zeros(n_rows,dtype = object)
    fL_NO = np.zeros(n_rows,dtype = object)
    
    fT_Chla = np.zeros(n_rows,dtype = object)
    fL_Chla = np.zeros(n_rows,dtype = object)
    
    volume_N = np.zeros(n_rows,dtype = object)
    volume_S = np.zeros(n_rows,dtype = object)
    ...
    Sim_Chla_N = np.zeros(n_rows,dtype = object)
    Sim_Chla_S = np.zeros(n_rows,dtype = object)
    Sim_Chla = np.zeros(n_rows,dtype = object)
    
    Chla_Load_Cal = np.zeros(n_rows,dtype = object)
    Chla_Load_StL = np.zeros(n_rows,dtype = object)
    Chla_Load_South = np.zeros(n_rows,dtype = object)
    ...
    Q_I = Q_in['Inflows_cmd'].astype(float) 
    Q_I_M = np.zeros(n_rows,dtype = object)
    ...
    volume_N[0] = StartStorage * 4046.85642 * 0.305 * N_Per #m3
    volume_N[1] = Stg_Sto_Ar.stg2sto(M_var.Lake_Stage[1],0) * 4046.85642 * 0.305 * N_Per #m3
    
    volume_S[0] = StartStorage * 4046.85642 * 0.305 * S_Per #m3
    volume_S[1] = Stg_Sto_Ar.stg2sto(M_var.Lake_Stage[1],0) * 4046.85642 * 0.305 * S_Per #m3
    ...
    #Generate the Zone Column based on the corresponding Zone Code.
        ...
        if Model_Config.Sim_type == 0:
            ...
            elif (Sim_Chla_S[i] <= Chla_1) and (NO_S[i] <= NO_1) and (TP_Lake_S[i] <= P_1) and (date_rng_6[i+2].month in [1,2,3,4,11,12]) and (M_var.LO_Zone[i+1] == 'B' or M_var.LO_Zone[i+1] == 'C'):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
            elif (Sim_Chla_S[i] <= Chla_2) and (NO_S[i] <= NO_2) and (TP_Lake_S[i] <= P_2) and (date_rng_6[i+2].month in [5,6,7,8,9,10]) and (M_var.LO_Zone[i+1] == 'B' or M_var.LO_Zone[i+1] == 'C'):
                M_var.Outlet2USRG[i+2] = S308_DV[(date_rng_6[i+2].month)-1]
        ...
        if Model_Config.Sim_type == 0:
            ...
            elif (Sim_Chla_S[i] <= Chla_1) and (NO_S[i] <= NO_1) and (TP_Lake_S[i] <= P_1) and (date_rng_6[i+2].month in [1,2,3,4,11,12]):
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]
            elif (Sim_Chla_S[i] <= Chla_2) and (NO_S[i] <= NO_2) and (TP_Lake_S[i] <= P_2) and (date_rng_6[i+2].month in [5,6,7,8,9,10]):
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]
            elif M_var.Lake_Stage[i+1] > LO_EcoRecEnv['Upper bound'].iloc[i+1] :  # Soft constraint
                M_var.Outlet1USREG[i+2] = S77_DV[(date_rng_6[i+2].month)-1]  
            else:
        ...
        volume_N[i+2] = M_var.Storage[i+2] * TP_Variables.N_Per * 4046.85642 * 0.305 #m3
        volume_S[i+2] = M_var.Storage[i+2] * TP_Variables.S_Per * 4046.85642 * 0.305 #m3
...
#Temp
Exported_File_Mean.to_csv('./Outputs/Exported_File_NOx_Chla_TP_Opt_Mean_%s_Scen4_StLMinQ.csv'%Pre_defined_Variables.Schedule)
Exported_File_Sum.to_csv('./Outputs/Exported_File_NOx_Chla_TP_Opt_Sum_%s_Scen4_StLMinQ.csv'%Pre_defined_Variables.Schedule)
