...
def Trib_HC():
    ...
    for i in range(TC_Count):
        ...
        M_var.MainTrib_Cls[i] = LONINO_FNs.MainTrib_Cls(
            Data.Wkly_Trib_Cond["S65E"].iloc[i]
        )
        M_var.Palmer_Cls[i] = LONINO_FNs.Palmer_Cls(
            Data.Wkly_Trib_Cond["Palmer"].iloc[i]
        )
        M_var.NetInflow_Cls[i] = LONINO_FNs.NetInflow_Cls(
    ...
    for i in range(Pre_defined_Variables.Month_N):
        M_var.LONINO_Seas_cls[i] = LONINO_FNs.LONINO_Seas_cls(
            LONINO_df["LONINO_Seas"].iloc[i]
        )
        M_var.LONINO_M_Seas_cls[i] = LONINO_FNs.LONINO_M_Seas_cls(
