...
class Data:
    ...
    SFWMM_Daily_Outputs = pd.read_csv(
        os.path.join(data_dir, f"SFWMM_Daily_Outputs_{schedule}.csv")
    )
    # read Water Shortage Management and Regulation Schedule Break points csv
    WSMs_RSBKs = pd.read_csv(
        os.path.join(data_dir, f"WSMs_RSBPs_{schedule}.csv")
    )
    # Read the weekly demand input file
    Weekly_dmd = pd.read_csv(
        os.path.join(data_dir, f"LOSA_wkly_dmd_{schedule}.csv")
    )
    ...
    Wkly_Trib_Cond = pd.read_csv(
        os.path.join(data_dir, f"Trib_cond_wkly_data_{schedule}.csv")
    )
    ...
    LONINO_Seas_data = pd.read_csv(
        os.path.join(data_dir, f"Seasonal_LONINO_{schedule}.csv")
    )
    LONINO_Mult_Seas_data = pd.read_csv(
        os.path.join(data_dir, f"Multi_Seasonal_LONINO_{schedule}.csv")
    )
    # read Netinflow data(ac-ft/day)
    NetInf_Input = pd.read_csv(
        os.path.join(data_dir, f"Netflows_acft_{schedule}.csv")
    )
    # read actual daily water demand (output of SFWMM)
    SFWMM_W_dmd = pd.read_csv(
        os.path.join(data_dir, f"Water_dmd_{schedule}.csv")
    )
    # read Rainfall Volume data
    RF_Vol = pd.read_csv(
        os.path.join(data_dir, f"RF_Volume_{schedule}.csv")
    )
    # read ET Vol data
    ET_Vol = pd.read_csv(os.path.join(data_dir, f"ETVol_{schedule}.csv"))
    # Read the C44 Runoff data which is output of SFWMM simulation.
    C44_Runoff = pd.read_csv(
        os.path.join(data_dir, f"C44RO_{schedule}.csv")
    )
    ...
    Sum_Basin_RO = pd.read_csv(
        os.path.join(data_dir, f"Basin_RO_inputs_{schedule}.csv")
    )
    # Read Mean Monthly basin runoffs (cfs)
    C43RO = pd.read_csv(
        os.path.join(data_dir, f"C43RO_Monthly_{schedule}.csv")
    )
    C44RO = pd.read_csv(
        os.path.join(data_dir, f"C44RO_Monthly_{schedule}.csv")
    )
    SLTRIB = pd.read_csv(
        os.path.join(data_dir, f"SLTRIB_Monthly_{schedule}.csv")
    )
    ...
    S77_RegRelRates = pd.read_csv(
        os.path.join(data_dir, f"S77_RegRelRates_{schedule}.csv")
    )
    S80_RegRelRates = pd.read_csv(
        os.path.join(data_dir, f"S80_RegRelRates_{schedule}.csv")
    )
    # Read the daily C43Runoff
    C43RO_Daily = pd.read_csv(
        os.path.join(data_dir, f"C43RO_{schedule}.csv")
    )
    ...
    CE_SLE_turns = pd.read_csv(
        os.path.join(data_dir, f"CE_SLE_turns_inputs_{schedule}.csv")
    )
    # Read the pulses input data (Tab Pulses in the LOOPS Spreadsheet Model!)
    Pulses = pd.read_csv(
        os.path.join(data_dir, f"Pulses_Inputs_{schedule}.csv")
    )
    ...
    Targ_Stg_June_1st = pd.read_csv(
        os.path.join(
            data_dir,
            f"Chance of June 1st Lake stage falling below 11.0ft_{schedule}"
            ".csv",
        ),
        parse_dates=["Date"],
    )
    Targ_Stg_May_1st = pd.read_csv(
        os.path.join(
            data_dir,
            f"Chance of May 1st Lake stage falling below 11.0ft_{schedule}"
            ".csv",
        ),
        parse_dates=["Date"],
    )
    ...
    Estuary_needs_water = pd.read_csv(
        os.path.join(data_dir, f"Estuary_needs_water_Input_{schedule}.csv")
    )
    # Read EAA_MIA_RUNOFF in cfs
    EAA_MIA_RUNOFF = pd.read_csv(
        os.path.join(data_dir, f"EAA_MIA_RUNOFF_Inputs_{schedule}.csv")
    )
    # Read calculated Storage deviation daily values
    Stroage_dev_df = pd.read_csv(
        os.path.join(data_dir, f"Storage_Dev_{schedule}.csv")
    )
