...
def LOONE_HydNut():
    ...
    for i in range(TC_Count):
        ...
        NetInflow_Cls[i] = LONINO_FNs.NetInflow_Cls(
    ...
    for i in range(LONINO_Count):
        Seas[i] = Data.LONINO_Seas_data["%s" % LONINO_df["date"].iloc[i].month].iloc[
            LONINO_df["date"].iloc[i].year - Pre_defined_Variables.startyear
        ]
        M_Seas[i] = Data.LONINO_Mult_Seas_data[
    ...
    for i in range(Pre_defined_Variables.Month_N):
        LONINO_Seas_cls[i] = LONINO_FNs.LONINO_Seas_cls(
            LONINO_df["LONINO_Seas"].iloc[i]
        )
        LONINO_M_Seas_cls[i] = LONINO_FNs.LONINO_M_Seas_cls(
    ...
    for i in range(num_B_R):
        ...
        C44RO_BS[i] = (
            max(0, Data.C44RO["C44RO"].iloc[i] - StlEst_baseflow) * Num_days[i]
        )
    ...
    Stage2ar[0] = utils.stg_sto_ar.stg2ar(Stage_LO[0], 0)
    Stage2ar[1] = utils.stg_sto_ar.stg2ar(Stage_LO[1], 0)
    ...
    for i in range(n_rows - 2):
        ...
        Max_Supply[i + 2] = LO_FNs.Max_Supply(
            WSM_Zone[i + 2],
            LOSA_dmd.at[i, "Daily_demand"],
        ...
        LOSA_Supply[i + 2] = LO_FNs.LOSA_Supply(
            WSM_Zone[i + 2],
        ...
        Zone_Code[i + 1] = LO_FNs.Zone_Code(
            Stage_LO[i + 1],
            df_WSMs.at[i + 1, "A"],
            df_WSMs.at[i + 1, "B"],
            df_WSMs.at[i + 1, "C"],
        ...
        #
        Zone_D_Trib[i] = utils.dec_tree_functions.Zone_D_Trib(
            TC_LONINO_df.at[i, "Tributary_Condition"], Pre_defined_Variables.Opt_NewTree
        )
        #
        Zone_D_stage[i] = utils.dec_tree_functions.Zone_D_stage(
            Stage_LO[i + 1], df_WSMs.at[i, "C-b"]
        )
        #
        Zone_D_Seas[i] = utils.dec_tree_functions.Zone_D_Seas(
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"],
            Zone_D_Trib[i],
            Pre_defined_Variables.Opt_NewTree,
        )
        #
        Zone_D_MSeas[i] = utils.dec_tree_functions.Zone_D_MSeas(
        ...
        #
        Zone_D_Rel_Code[i] = utils.dec_tree_functions.Zone_D_Rel_Code(
            Zone_D_Branch_Code[i], Pre_defined_Variables.Opt_DecTree
        )
        #
        Zone_C_Trib[i] = utils.dec_tree_functions.Zone_C_Trib(
            TC_LONINO_df.at[i, "Tributary_Condition"], Pre_defined_Variables.Opt_NewTree
        )
        #
        Zone_C_Seas[i] = utils.dec_tree_functions.Zone_C_Seas(
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"],
            Pre_defined_Variables.Opt_NewTree,
        )
        #
        Zone_C_MSeas[i] = utils.dec_tree_functions.Zone_C_MSeas(
            TC_LONINO_df.at[i, "LONINO_MultiSeasonal_Classes"]
        )
        #
        Zone_C_MetFcast[i] = utils.dec_tree_functions.Zone_C_MetFcast(
            Zone_C_Seas[i],
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"],
        ...
        ZCBC = (
            Zone_C_Trib[i] * 1000
            + Zone_C_MetFcast[i] * 100
            + Zone_C_Seas[i] * 10
            + Zone_C_MSeas[i] * 1
        )
        Zone_C_Branch_Code[i] = ZCBC
        #
        Zone_C_Rel_Code[i] = utils.dec_tree_functions.Zone_C_Rel_Code(
            Zone_C_Branch_Code[i], Pre_defined_Variables.Opt_DecTree
        )
        #
        Zone_B_Trib[i] = utils.dec_tree_functions.Zone_B_Trib(
            TC_LONINO_df.at[i, "Tributary_Condition"], Pre_defined_Variables.Opt_NewTree
        )
        #
        Zone_B_Stage[i] = utils.dec_tree_functions.Zone_B_Stage(
            Stage_LO[i + 1], Seasons.at[i, "Season"]
        )
        #
        Zone_B_Seas[i] = utils.dec_tree_functions.Zone_B_Seas(
            TC_LONINO_df.at[i, "LONINO_Seasonal_Classes"]
        )
        #
        ZBBC = (
        ...
        #
        Zone_B_Rel_Code[i] = utils.dec_tree_functions.Zone_B_Rel_Code(
            Zone_B_Branch_Code[i], Pre_defined_Variables.Opt_DecTree
        )
        # Generate the DecTree Release level
        DecTree_Relslevel[i + 2] = LO_FNs.DecTree_Relslevel(
            Zone_Code[i + 1], Zone_D_Rel_Code[i], Zone_C_Rel_Code[i], Zone_B_Rel_Code[i]
        )
        # Calculate DayFlags
        if i >= 3:
            if (
            ...
            DayFlags[i] = X2
        #
        PlsDay[i + 2] = LO_FNs.PlsDay(
            DayFlags[i + 2],
            DecTree_Relslevel[i + 2],
            Pre_defined_Variables.PlsDay_Switch,
        )
        # Calculate the Release Level Column
        Release_Level[i + 2] = LO_FNs.Release_Level(
            Stage_LO[i + 1],
        ...
        ZoneCodeminus1Code[i + 1] = LO_FNs.ZoneCodeminus1Code(
            Zone_Code[i + 1],
            df_WSMs.at[i + 1, "WSM1"],
            df_WSMs.at[i + 1, "D0"],
            df_WSMs.at[i + 1, "D1"],
        ...
        ZoneCodeCode[i + 1] = LO_FNs.ZoneCodeCode(
            Zone_Code[i + 1],
            df_WSMs.at[i + 1, "WSM1"],
            df_WSMs.at[i + 1, "D0"],
            df_WSMs.at[i + 1, "D1"],
        ...
        Fraction_of_Zone_height[i + 1] = LO_FNs.Fraction_of_Zone_height(
            Zone_Code[i + 1],
            Stage_LO[i + 1],
            ZoneCodeminus1Code[i + 1],
            ZoneCodeCode[i + 1],
        )
        # Define three Codes based on the RelsLevel that will be used to calculate S80_Mult!
        ReLevelCode_1[i + 2] = LO_FNs.ReLevelCode_1(
            Release_Level[i + 2],
        ...
        #
        ReLevelCode_2[i + 2] = LO_FNs.ReLevelCode_2(
            Release_Level[i + 2],
        ...
        #
        ReLevelCode_3_S80[i + 2] = LO_FNs.ReLevelCode_3_S80(
            Release_Level[i + 2],
        ...
        S80_Mult[i + 2] = LO_FNs.S80_Mult(
            Seasons.at[i, "Season"],
            Seasons.at[i, "Month"],
            dh_7days[i + 1],
            ReLevelCode_1[i + 2],
            Fraction_of_Zone_height[i + 1],
            ReLevelCode_2[i + 2],
            ReLevelCode_3_S80[i + 2],
        ...
        S80_Mult_2[i + 2] = LO_FNs.S80_Mult_2(
            LO_Model.at[i + 2, "date"].month,
            LO_Model.at[i + 2, "date"].day,
            PlsDay[i + 2],
            S80_Mult[i + 2 - PlsDay[i + 2]],
            S80_Mult[i + 2],
            Pre_defined_Variables.Opt_QregMult,
        )
        ...
        S80RS[i + 2] = LO_FNs.S80RS(
            ...
            S80avgL1,
            Data.Pulses.at[
                PlsDay[i + 2] - 1 if PlsDay[i + 2] - 1 >= 0 else len(Data.Pulses) - 1,
                "S-80_L1",
            ],
            S80_Mult_2[i + 2],
        ...
        Sum_S308RG1[i + 2] = LO_FNs.Sum_S308RG1(
            LO_Model.at[i + 2, "date"].day, S308RG1[i + 2]
        )
        # S80 Baseflow Target
        ...
        Sum_S308BK[i + 2] = LO_FNs.Sum_S308BK(
            LO_Model.at[i + 2, "date"].day, S308BK[i + 2]
        )
        # Calculate S308RG
        # Define a Choose function to be used to calculate S308RG.
        S308RG_Code[i + 2] = LO_FNs.S308RG_Code(
            S308RG1[i + 2],
            S308BS[i + 2],
        ...
        ReLevelCode_3_S77[i + 2] = LO_FNs.ReLevelCode_3_S77(
            Release_Level[i + 2],
        ...
        S77_Mult_2[i + 2] = LO_FNs.S77_Mult_2(
            LO_Model.at[i + 2, "date"].month,
            LO_Model.at[i + 2, "date"].day,
            PlsDay[i + 2],
            S77_Mult[i + 2 - PlsDay[i + 2]],
            S77_Mult[i + 2],
            Pre_defined_Variables.Opt_QregMult,
        )
        # Calculate S77RS (Reg Schedule Release without BaseFlow).
        S77RS[i + 2] = LO_FNs.S77RS(
            Release_Level[i + 2],
            ...
            Pre_defined_Variables.Opt_S79RG,
        )
        # Calculale accumulated S77RS
        Sum_S77RS[i + 2] = LO_FNs.Sum_S77RS(
            LO_Model.at[i + 2, "date"].day, S77RS[i + 2]
        )
        # S-77 backflow logic:
        ...
        S77BK[i + 2] = LO_FNs.S77BK(
            ...
            Pre_defined_Variables.S77BK_Switch,
            Pre_defined_Variables.S77BK_Threshold,
        )
        # ROwest
        ...
        if i < (n_rows - 2):
            Post_Ap_Baseflow[i] = utils.thc_class.THC_Class(
                i,
            ...
            Post_AP_EWS[i] = utils.thc_class.THC_Class(
                i,
                THC_Class_normal_or_above,
                Lake_O_Stage_AP,
                Lake_O_Schedule_Zone,
        ...
        S77BSAP[i + 2] = LO_FNs.S77BSAP(
            S77BS[i + 2], Post_Ap_Baseflow[i], Pre_defined_Variables.Opt_AdapProt
        )
        # THE EWS COMPONENT WAS  SPLIT OUT FOR ACCOUNTING PURPOSES SINCE EWS IS NOT A REG RELEASE.  MAY2013.
        S77EWS[i + 2] = LO_FNs.S77EWS(
            Post_AP_EWS[i],
            Data.SFWMM_Daily_Outputs.at[i + 2, "CAEST"],
            Pre_defined_Variables.S77EWS_Switch,
        ...
        RegWCA[i + 2] = min(
            ...
            Pre_defined_Variables.Multiplier_RegWCA * Choose_WCA[i + 2],
        )
        # Define Choose function for RegL8C51
        Choose_L8C51[i + 2] = LO_FNs.Choose_L8C51(
        ...
        RegL8C51[i + 2] = min(
            ...
            Pre_defined_Variables.Multiplier_RegL8C51 * Choose_L8C51[i + 2],
        )
        # TotRegSo
        ...
        # Calculate stg2ar(ac)
        Stage2ar[i + 2] = utils.stg_sto_ar.stg2ar(Stage_LO[i + 1], 0)
        # Calculate stg2mar(ac)
        Stage2marsh[i + 2] = utils.stg_sto_ar.stg2mar(Stage_LO[i + 1], 0)
        # RF(af)
        ...
        Storage[i + 2] = LO_FNs.Storage(
            DayFlags[i + 2], Storage[i], StartStorage, Storage[i + 1], DSto[i + 2]
        )
        # Lake Okeechobee Stage!
        Stage_LO[i + 2] = LO_FNs.Stage_LO(
            utils.stg_sto_ar.stg2sto(Storage[i + 2], 1),
        ...
        Q_O[i + 2] = (
            (
                S77EWS[i + 2] * 0.028316847
                + ((TotRegEW[i + 2] + TotRegSo[i + 2]) / 70.0456)
            )
            * 3600
            * 24
        )
        ...
        Stage2ar[i + 2] = utils.stg_sto_ar.stg2ar(Stage_LO[i + 2], 0)
