...
class Pre_defined_Variables:
    ...
    MaxQstgTrigger = (
        20  # the maximum stage trigger for maximum discharge if Trib_cond. = XWet.
    )
    Opt_QregMult = 0  # option for using multipliers 0: don't use, 1: apply only during dry season (Nov-May), 2: apply year-round.
    AlternateHighQyrs = 0
    Option_S80Baseflow = (
        0  # 0: baseflow supplements daily C44RO, 1: baseflow supplements monthly C44RO.
    )
    S308BK_Const = (
        1  # method to calculate S308BK 0: Use Input data or 1: Simulate S308BK.
    )
    S308_BK_Thr = 14.5  # threshold for S308 Backflow to occur.
    Opt_S308 = 1  # option for using S308 for LOREG and Backflow (1: Y and 0: N).
    S308RG_Const = (
        1  # 0: use Input data (overrides Option_RegS77S308) and 1: simulate S308RG.
    )
    ...
    Outlet1USBK_Switch = (
        1  # option for S77BK simulation 0: Use input data or 1: Simulate with LOONE.
    )
    Outlet1USBK_Threshold = 11.1
    Option_S77Baseflow = (
        0  # 0: baseflow supplements daily C43RO, 1: baseflow supplements monthly C43RO.
    )
